...
const emptyPackageItem = { partNumber: "", sapCode: "", description: "", quantity: "", unitType: "", unitPrice: "", overhead: "" };
...
function QuotationWorkspace({project,packages,items,selectedPackageId,setSelectedPackageId,showForm,setShowForm,packageForm,setPackageForm,itemForm,setItemForm,createPackage,saveItem,updateStatus,editingItemId,startEditItem,cancelEditItem,deleteItem,deletePackage,clonePackage,togglePackageTotal}) {
  ...
  const total=(pkgItems)=>pkgItems.reduce((sum,item)=>sum+Number(item.unit_price||0)*Number(item.quantity||0)*(1+Number(item.overhead_rate||0)/100),0);
  const selectedForTotal=projectPackages.filter(pkg=>pkg.include_in_total);
  const totalsByCurrency=selectedForTotal.reduce((totals,pkg)=>{totals[pkg.currency]=(totals[pkg.currency]||0)+total(items.filter(item=>item.package_id===pkg.id));return totals;},{});
  ...
  return <section className="quotation-workspace">
    ...
    {showForm&&<form className="quotation-package-form simplified" onSubmit={createPackage}><input required value={packageForm.name} onChange={e=>setPackageForm({...packageForm,name:e.target.value})} placeholder="Nome do pacote"/><input value={packageForm.supplier} onChange={e=>setPackageForm({...packageForm,supplier:e.target.value})} placeholder="Fornecedor (opcional)"/><select value={packageForm.currency} onChange={e=>setPackageForm({...packageForm,currency:e.target.value})}><option value="BRL">BRL</option><option value="USD">USD</option></select><input type="date" value={packageForm.dueDate} onChange={e=>setPackageForm({...packageForm,dueDate:e.target.value})}/><button>Criar pacote</button></form>}
    <div className="quotation-total-bar"><div><span>Total selecionado</span><small>{selectedForTotal.length} de {projectPackages.length} pacotes incluídos</small></div><div>{Object.entries(totalsByCurrency).map(([currency,value])=><strong key={currency}>{format(value,currency)}</strong>)}{selectedForTotal.length===0&&<strong>Selecione os pacotes que devem somar</strong>}</div></div>
    <div className="quotation-layout"><aside>{projectPackages.map(pkg=>{const packageItems=items.filter(item=>item.package_id===pkg.id);return <div className={`quotation-package-card ${selectedPackageId===pkg.id?"active":""}`} key={pkg.id}><button className="quotation-package-select" onClick={()=>{setSelectedPackageId(pkg.id);cancelEditItem()}}><span><strong>{pkg.name}</strong><small>{packageItems.length} componentes · {pkg.status}</small></span><b>{format(total(packageItems),pkg.currency)}</b></button><label className="quotation-total-check"><input type="checkbox" checked={Boolean(pkg.include_in_total)} onChange={e=>togglePackageTotal(pkg,e.target.checked)}/> Somar</label><div className="quotation-package-actions"><button title="Clonar pacote" onClick={()=>clonePackage(pkg)}>Clonar</button><button className="delete" title="Excluir pacote" onClick={()=>deletePackage(pkg)}>Excluir</button></div></div>})}</aside>
      {selected?<div className="quotation-detail"><header><div><strong>{selected.name}</strong><small>{selected.supplier||"Fornecedor não definido"}</small></div><select value={selected.status} onChange={e=>updateStatus(selected,e.target.value)}><option value="draft">Em elaboração</option><option value="waiting_supplier">Aguardando fornecedor</option><option value="received">Recebida</option><option value="analysis">Em análise</option><option value="approved">Aprovada</option><option value="rejected">Reprovada</option></select></header>
        <form className={`quotation-item-form revised ${editingItemId?"editing":""}`} onSubmit={saveItem}><input required value={itemForm.description} onChange={e=>setItemForm({...itemForm,description:e.target.value})} placeholder="Componente ou serviço"/><input required aria-label="Quantidade" min=".01" step=".01" type="number" value={itemForm.quantity} onChange={e=>setItemForm({...itemForm,quantity:e.target.value})} placeholder="Quantidade"/><select required aria-label="Unidade" value={itemForm.unitType} onChange={e=>setItemForm({...itemForm,unitType:e.target.value})}><option value="" disabled>Unidade</option>{["UN","PC","KIT","CX","KG","M","L","H"].map(u=><option key={u}>{u}</option>)}</select><input min="0" required step=".01" type="number" value={itemForm.unitPrice} onChange={e=>setItemForm({...itemForm,unitPrice:e.target.value})} placeholder="Valor unitário"/><input min="0" step=".01" type="number" value={itemForm.overhead} onChange={e=>setItemForm({...itemForm,overhead:e.target.value})} placeholder="Overhead %"/><input value={itemForm.partNumber} onChange={e=>setItemForm({...itemForm,partNumber:e.target.value})} placeholder="P/N (opcional)"/><input value={itemForm.sapCode} onChange={e=>setItemForm({...itemForm,sapCode:e.target.value})} placeholder="Código SAP (opcional)"/><div className="quotation-form-actions"><button>{editingItemId?"Salvar alterações":"Adicionar componente"}</button>{editingItemId&&<button type="button" className="cancel" onClick={cancelEditItem}>Cancelar</button>}</div></form>
        <div className="quotation-items">{selectedItems.map(item=><div key={item.id}><span><strong>{item.description}</strong><small>{item.quantity} {item.unit_type} · Overhead {item.overhead_rate||0}%{item.item_code?` · P/N ${item.item_code}`:""}{item.sap_code?` · SAP ${item.sap_code}`:""}</small></span><b>{format(total([item]),selected.currency)}</b><div className="quotation-item-actions"><button onClick={()=>startEditItem(item)}>Editar</button><button className="delete" onClick={()=>deleteItem(item)}>Excluir</button></div></div>)}</div>
...
export default function NewProductsDashboard({ onOpenProducts }) {
  ...
  async function createQuotationPackage(event) {
    event.preventDefault(); const {data,error}=await supabase.from("quotation_packages").insert({project_id:selectedProject.id,name:packageForm.name.trim(),supplier:packageForm.supplier.trim()||null,currency:packageForm.currency,due_date:packageForm.dueDate||null,include_in_total:false}).select("*").single();
  ...
  async function cloneQuotationPackage(pkg){const sourceItems=quotationItems.filter(item=>item.package_id===pkg.id);const{data:clone,error}=await supabase.from("quotation_packages").insert({project_id:pkg.project_id,name:`${pkg.name} (cópia)`,supplier:pkg.supplier,currency:pkg.currency,due_date:pkg.due_date,status:"draft",include_in_total:false}).select("*").single();if(error){setMessage(error.message);return;}let clonedItems=[];if(sourceItems.length){const{data,error:itemError}=await supabase.from("quotation_package_items").insert(sourceItems.map(item=>({package_id:clone.id,item_code:item.item_code,sap_code:item.sap_code,description:item.description,quantity:item.quantity,unit_type:item.unit_type,unit_price:item.unit_price,mkp:1,overhead_rate:item.overhead_rate}))).select("*");if(itemError){setMessage(`Pacote clonado, mas os componentes falharam: ${itemError.message}`);}else clonedItems=data??[];}setQuotationPackages(current=>[clone,...current]);setQuotationItems(current=>[...current,...clonedItems]);setSelectedPackageId(clone.id);cancelEditQuotationItem();setMessage("Pacote clonado. Revise os dados antes de aprovar.");}
