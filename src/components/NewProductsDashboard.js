  ...
  return <section className="quotation-workspace">
    ...
    <div className="quotation-layout"><aside>{projectPackages.map(pkg=>{const packageItems=items.filter(item=>item.package_id===pkg.id);return <div className={`quotation-package-card ${selectedPackageId===pkg.id?"active":""}`} key={pkg.id}><button className="quotation-package-select" onClick={()=>{setSelectedPackageId(pkg.id);cancelEditItem()}}><span><strong>{pkg.name}</strong><small>{packageItems.length} componentes · {pkg.status}</small></span><b>{format(total(packageItems),"BRL")}</b></button><label className="quotation-total-check"><input type="checkbox" checked={Boolean(pkg.include_in_total)} onChange={e=>togglePackageTotal(pkg,e.target.checked)}/> Somar</label><div className="quotation-package-actions"><button title="Clonar pacote" onClick={()=>clonePackage(pkg)}>Clonar</button><button className="delete" title="Excluir pacote" onClick={()=>deletePackage(pkg)}>Excluir</button></div></div>})}</aside>
      {selected?<div className="quotation-detail"><header><div><strong>{selected.name}</strong><small>{selected.supplier||"Fornecedor não definido"}</small></div><select value={selected.status} onChange={e=>updateStatus(selected,e.target.value)}><option value="draft">Em elaboração</option><option value="waiting_supplier">Aguardando fornecedor</option><option value="received">Recebida</option><option value="analysis">Em análise</option><option value="approved">Aprovada</option><option value="rejected">Reprovada</option></select></header>
        <form className={`quotation-item-form revised financial ${editingItemId?"editing":""}`} onSubmit={saveItem}><input required value={itemForm.description} onChange={e=>setItemForm({...itemForm,description:e.target.value})} placeholder="Componente ou serviço"/><input required aria-label="Quantidade" min=".01" step=".01" type="number" value={itemForm.quantity} onChange={e=>setItemForm({...itemForm,quantity:e.target.value})} placeholder="Quantidade"/><select required aria-label="Unidade" value={itemForm.unitType} onChange={e=>setItemForm({...itemForm,unitType:e.target.value})}><option value="" disabled>Unidade</option>{["UN","PC","KIT","CX","KG","M","L","H"].map(u=><option key={u}>{u}</option>)}</select><input min="0" required step=".01" type="number" value={itemForm.unitPrice} onChange={e=>setItemForm({...itemForm,unitPrice:e.target.value})} placeholder="Valor unitário"/><select required value={itemForm.currency} onChange={e=>setItemForm({...itemForm,currency:e.target.value,exchangeRate:e.target.value==="BRL"?"1":""})}><option value="" disabled>Moeda</option><option value="BRL">BRL</option><option value="USD">USD</option></select>{itemForm.currency==="USD"&&<input min="0.01" required step=".0001" type="number" value={itemForm.exchangeRate} onChange={e=>setItemForm({...itemForm,exchangeRate:e.target.value})} placeholder="Cotação USD"/>}<input min="0" step=".01" type="number" value={itemForm.overhead} onChange={e=>setItemForm({...itemForm,overhead:e.target.value})} placeholder="Overhead %"/><input list="quotation-ncm" value={itemForm.ncm} onChange={e=>setItemForm({...itemForm,ncm:e.target.value})} placeholder="NCM (opcional)"/><datalist id="quotation-ncm">{ncmTaxes.map(tax=><option key={tax.id} value={tax.ncm}>{tax.description}</option>)}</datalist>{itemForm.ncm&&<div className="quotation-tax-options">{[["applyIpi","IPI","ipi_rate"],["applyPis","PIS","pis_rate"],["applyCofins","COFINS","cofins_rate"],["applyIcms","ICMS","icms_rate"],["applyImportTax","Importação","import_tax_rate"]].map(([key,label,rate])=><label key={key}><input type="checkbox" checked={itemForm[key]} onChange={e=>setItemForm({...itemForm,[key]:e.target.checked})}/>{label} {Number(ncmTaxes.find(t=>t.ncm===itemForm.ncm)?.[rate]||0)}%</label>)}</div>}<input value={itemForm.partNumber} onChange={e=>setItemForm({...itemForm,partNumber:e.target.value})} placeholder="P/N (opcional)"/><input value={itemForm.sapCode} onChange={e=>setItemForm({...itemForm,sapCode:e.target.value})} placeholder="Código SAP (opcional)"/><div className="quotation-form-actions"><button>{editingItemId?"Salvar alterações":"Adicionar componente"}</button>{editingItemId&&<button type="button" className="cancel" onClick={cancelEditItem}>Cancelar</button>}</div></form>
...
export default function NewProductsDashboard({ onOpenProducts }) {
  ...
  async function createQuotationPackage(event) {
    event.preventDefault(); const {data,error}=await supabase.from("quotation_packages").insert({project_id:selectedProject.id,name:packageForm.name.trim(),supplier:packageForm.supplier.trim()||null,currency:"BRL",due_date:packageForm.dueDate||null,include_in_total:false}).select("*").single();
