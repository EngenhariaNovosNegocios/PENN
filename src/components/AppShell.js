  ...
  return (
    <div className={`app-shell ${collapsed ? "sidebar-collapsed" : ""}`}>
      <aside className={`app-sidebar ${menuOpen ? "mobile-open" : ""}`}>
        ...
        <nav className="primary-nav" aria-label="Navegação principal"><button className={`nav-item personal-tasks-nav ${activePage === "profile" ? "active" : ""}`} onClick={()=>{setActivePage("profile");setMenuOpen(false)}} title={collapsed?"Minhas tarefas":undefined}><Icon name="tasks"/><span>Minhas tarefas</span>{overdueTasks.length>0&&<small>{overdueTasks.length}</small>}</button><span className="nav-section-label">Workspace</span>{navigation.map((item) => <button className={`nav-item ${activePage === item.id ? "active" : ""}`} disabled={item.disabled} key={item.label} onClick={() => { if (item.id) setActivePage(item.id); setMenuOpen(false); }} title={collapsed ? item.label : undefined}><Icon name={item.icon}/><span>{item.label}</span>{item.disabled && <small>Em breve</small>}</button>)}</nav>
